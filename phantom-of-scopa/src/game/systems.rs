...
use crate::error::{Result, ScopaError};
use crate::popups::*;
use crate::styles::*;
use scopa_lib::card::*;
...
pub fn game_setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let take_button = (
        ImageNode::new(take_button_image),
        Node {
            position_type: PositionType::Absolute,
            left: Val::Px(669.0),
            top: Val::Px(187.0),
    ...
    commands
        ...
        .with_children(|parent| {
            parent.spawn((
                ImageNode::new(take_button_highlight_image),
                Visibility::Hidden,
                HighlightImage,
            ));
        });
    ...
    // let put_button = (
    //     ImageNode::new(put_button_image),
    //     Node {
    //         position_type: PositionType::Absolute,
    //         left: Val::Px(669.0),
    //         top: Val::Px(242.0),
    ...
    //     .with_children(|parent| {
    //         parent.spawn((
    //             ImageNode::new(put_button_highlight_image),
    //             Visibility::Hidden,
    //             HighlightImage,
    //         ));
    //     });
    ...
    //             ImageNode::new(asset_server.load("table_highlight.png")),
    //             Node {
    //                 width: Val::Px(TABLE_WIDTH),
    //                 height: Val::Px(TABLE_HEIGHT),
    //                 ..default()
...
// fn create_table_slot(parent: &mut Commands, row: usize, column: usize) -> Entity {
...
pub fn take_button_pressed(
    ...
    for interaction in &interaction_query {
        if let Interaction::Pressed = *interaction {
            game_events.write(GameEvent::NewHand(random_hand()));
        }
    }
}
...
//             if let Ok(card_id) = player_selected_card.single() {
...
//                     commands.entity(slot.id()).despawn_children();
//                     slot.free();
//                 }
//             }
//         }
...
//         card.insert(ChildOf(slot_id));
//         Ok(())
//     } else {
//         Err(ScopaError::Gameplay("The table is full".into()))
//     }
// }
...
pub fn update_hand(
    ...
    mut slot_image_query: Query<&mut ImageNode, With<PlayerCard>>,
    asset_server: Res<AssetServer>,
    mut commands: Commands,
    config: Res<Config>,
) {
        ...
        match event {
            GameEvent::NewHand(hand) => {
                ...
                for ((slot, slot_children), card) in hand_slots_query.iter_mut().zip(cards) {
                    let new_card_image = ImageNode::new(asset_server.load(card.asset_path()));
                        ...
                        *card_image = new_card_image;
                        ...
                        let new_slot_image = commands
                            .spawn((
                                ...
                                RelativeCursorPosition::default(),
                                Interaction::None,
                                ImageNode::new(new_card_image.image),
                                Node {
...
pub fn select_hand_card(
    ...
    if let Ok((id, Interaction::Pressed)) = interacted_card_query.single() {
        ...
        // time
        if let Ok(prev_selected_id) = selected_card_query.single() {
...
fn create_image(image: Handle<Image>) -> impl Bundle {
    (
        ImageNode::new(image),
        Node {
            width: Val::Px(CARD_WIDTH),
...
//         let selected_image_id = commands.spawn(create_image(selected_image.0.clone())).id();
...
//         deselected_card.despawn_children();
//     }
// }
